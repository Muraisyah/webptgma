@extends('layouts.app')
@section('title','Tambah Jemaah')
@section('content')
<h3>Tambah Jemaah</h3>
@include('partials.alerts')
<form action="{{ route('admin.data-jemaah.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    {{-- id_user otomatis dari authenticated user --}}
    <div class="mb-3"><label class="form-label">Nama</label><input name="nama_jemaah" class="form-control" value="{{ old('nama_jemaah') }}" required maxlength="35"></div>
    <div class="mb-3"><label class="form-label">Tempat Lahir</label><input name="tempat_lahir" class="form-control" value="{{ old('tempat_lahir') }}" maxlength="30"></div>
    <div class="mb-3"><label class="form-label">Tanggal Lahir</label><input id="tgl_lahir" type="text" name="tgl_lahir" class="form-control flatpickr" value="{{ old('tgl_lahir') }}"></div>
    <div class="mb-3"><label class="form-label">NIK</label><input name="nik" class="form-control" value="{{ old('nik') }}" maxlength="16"></div>
    <div class="mb-3"><label class="form-label">Jenis Kelamin</label><select name="jenis_kelamin" class="form-select"><option value="">-</option><option value="Laki-laki">Laki-laki</option><option value="Perempuan">Perempuan</option></select></div>
    <div class="mb-3"><label class="form-label">Alamat</label><textarea name="alamat" class="form-control">{{ old('alamat') }}</textarea></div>
    <div class="mb-3"><label class="form-label">Nama Ayah</label><input name="nama_ayah" class="form-control" value="{{ old('nama_ayah') }}" maxlength="35"></div>
    ...
    <div class="mb-3"><label class="form-label">Kewarganegaraan</label><input name="kewarganegaraan" class="form-control" value="{{ old('kewarganegaraan') }}" maxlength="30"></div>
    ...
    <div id="buku-nikah-section" style="display:{{ old('status_pernikahan')=='Menikah' ? 'block':'none' }}">
        <div class="mb-3"><label class="form-label">Foto Buku Nikah</label><input type="file" name="foto_buku_nikah" class="form-control"></div>
    </div>
    <div id="parent-ktp-section" style="display:none">
        ...
        <select id="select_passport" name="id_passport" class="form-select mb-2 select-search">
            ...
            @foreach($passports as $p)
                <option value="{{ $p->id_passport }}"
                    ...
                    {{ old('id_passport') == $p->id_passport ? 'selected' : '' }}>
                    {{ ($p->nomor_passport ? $p->nomor_passport.' - ' : '').($p->nama_passport ?? $p->nama_tambahan) }}
                </option>
            @endforeach
        </select>
        <div class="mb-2"><img id="passport_preview_img" src="" alt="Preview Passport" style="max-width:180px;display:none;border:1px solid #ddd;padding:4px;border-radius:4px"></div>
        </div>
        <select id="passport_have" class="form-select">
            <option value="0" {{ old('id_passport') ? '' : 'selected' }}>Belum punya</option>
            <option value="1" {{ old('id_passport') ? 'selected' : '' }}>Sudah punya</option>
        </select>
    </div>
    <div id="passport-section" style="display:none">
        <h6>Data Passport (isi jika ada)</h6>
        <div class="mb-3"><label class="form-label">Nama Passport</label><input name="passport_nama_passport" class="form-control" value="{{ old('passport_nama_passport') }}"></div>
        <div class="mb-3"><label class="form-label">Nama Tambahan</label><input name="passport_nama_tambahan" class="form-control" value="{{ old('passport_nama_tambahan') }}"></div>
        <div class="mb-3"><label class="form-label">Nomor Passport</label><input name="passport_nomor_passport" class="form-control" value="{{ old('passport_nomor_passport') }}"></div>
        <div class="mb-3"><label class="form-label">Tempat Lahir (Passport)</label><input name="passport_tempat_lahir_pass" class="form-control" value="{{ old('passport_tempat_lahir_pass') }}"></div>
        <div class="mb-3"><label class="form-label">Tanggal Lahir (Passport)</label><input type="text" name="passport_tgl_lahir_pass" class="form-control flatpickr" value="{{ old('passport_tgl_lahir_pass') }}"></div>
        <div class="mb-3"><label class="form-label">Tempat Pembuatan</label><input name="passport_tempat_pembuatan" class="form-control" value="{{ old('passport_tempat_pembuatan') }}"></div>
        <div class="mb-3"><label class="form-label">Tanggal Pembuatan</label><input type="text" name="passport_tgl_pembuatan" class="form-control flatpickr" value="{{ old('passport_tgl_pembuatan') }}"></div>
        <div class="mb-3"><label class="form-label">Tanggal Exp Passport</label><input type="text" name="passport_exp_passport" class="form-control flatpickr" value="{{ old('passport_exp_passport') }}"></div>
        ...
        <div class="mb-3"><label class="form-label">Status Passport</label><select name="passport_status_passport" class="form-select"><option value="Aktif">Aktif</option><option value="Expired">Expired</option></select></div>
    </div>
        ...
        <select id="select_visa" name="id_visa" class="form-select mb-2 select-search">
            ...
            @foreach($visas as $v)
                <option value="{{ $v->id_visa }}"
                    ...
                    {{ old('id_visa') == $v->id_visa ? 'selected' : '' }}>
                    {{ ($v->nomor_visa ? $v->nomor_visa.' - ' : '').($v->nama_visa ?? '') }}
                </option>
            @endforeach
        </select>
        <div class="mb-2"><img id="visa_preview_img" src="" alt="Preview Visa" style="max-width:180px;display:none;border:1px solid #ddd;padding:4px;border-radius:4px"></div>
        </div>
        <select id="visa_have" class="form-select">
            <option value="0" {{ old('id_visa') ? '' : 'selected' }}>Belum punya</option>
            <option value="1" {{ old('id_visa') ? 'selected' : '' }}>Sudah punya</option>
        </select>
    </div>
    <div id="visa-section" style="display:none">
        <h6>Data Visa (isi jika ada)</h6>
        <div class="mb-3"><label class="form-label">Nama Visa</label><input name="visa_nama_visa" class="form-control" value="{{ old('visa_nama_visa') }}"></div>
        <div class="mb-3"><label class="form-label">Nomor Visa</label><input name="visa_nomor_visa" class="form-control" value="{{ old('visa_nomor_visa') }}"></div>
        <div class="mb-3"><label class="form-label">Tanggal Berlaku Visa</label><input type="text" name="visa_tgl_berlaku_visa" class="form-control flatpickr" value="{{ old('visa_tgl_berlaku_visa') }}"></div>
        <div class="mb-3"><label class="form-label">Tanggal Exp Visa</label><input type="text" name="visa_tgl_exp_visa" class="form-control flatpickr" value="{{ old('visa_tgl_exp_visa') }}"></div>
        ...
        <select id="select_vaksin" name="id_vaksin" class="form-select mb-2 select-search">
            ...
            @foreach($vaksins as $k)
                <option value="{{ $k->id_vaksin }}"
                    ...
                    {{ old('id_vaksin') == $k->id_vaksin ? 'selected' : '' }}>
                    {{ $k->nama_vaksin }}
                </option>
            @endforeach
        </select>
        <div class="mb-2"><img id="vaksin_preview_img" src="" alt="Preview Vaksin" style="max-width:180px;display:none;border:1px solid #ddd;padding:4px;border-radius:4px"></div>
        </div>
        <select id="vaksin_have" class="form-select">
            <option value="0" {{ old('id_vaksin') ? '' : 'selected' }}>Belum punya</option>
            <option value="1" {{ old('id_vaksin') ? 'selected' : '' }}>Sudah punya</option>
        </select>
    </div>
    <div id="vaksin-section" style="display:none">
        <h6>Data Vaksin (isi jika ada)</h6>
        <div class="mb-3"><label class="form-label">Nama Vaksin</label><input name="vaksin_nama_vaksin" class="form-control" value="{{ old('vaksin_nama_vaksin') }}"></div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if(selPassport){ selPassport.addEventListener('change', function(e){ const o = this.options[this.selectedIndex]; if(this.value) fillPassportFieldsFromOption(o); });
        // populate on load if there is a selected option
        if(selPassport.value){ fillPassportFieldsFromOption(selPassport.options[selPassport.selectedIndex]); }
    }
    const selVisa = document.getElementById('select_visa');
    if(selVisa){ selVisa.addEventListener('change', function(e){ const o = this.options[this.selectedIndex]; if(this.value) fillVisaFieldsFromOption(o); }); if(selVisa.value){ fillVisaFieldsFromOption(selVisa.options[selVisa.selectedIndex]); } }
    const selVaksin = document.getElementById('select_vaksin');
    if(selVaksin){ selVaksin.addEventListener('change', function(e){ const o = this.options[this.selectedIndex]; if(this.value) fillVaksinFieldsFromOption(o); }); if(selVaksin.value){ fillVaksinFieldsFromOption(selVaksin.options[selVaksin.selectedIndex]); } }
});
</script>
@endsection
